...
def create_analysis_prompt(pdf_text, search_results, is_first_ordinance=False):
    ...
    prompt += (
        ...
        "- 개선이 필요한 부분과 그 방향성\n"
        "4. [입법컨설팅 제안] (별도 소제목)\n"
        "조례안 문구 중에 아래 입법컨설팅 사례에 근거해서 수정할만한 내용이 있으면 작성해줘.\n"
        "1. 상위 법령과의 정합성 확보 :\n"
        "- 불필요한 중복 삭제: 조례에 명시할 실익이 낮거나 상위 법령에 이미 규정된 내용은 삭제를 제안합니다. 예를 들어, '「재난 및 안전관리 기본법」에 따른 재난의 정의'와 같이 상위 법령을 인용하는 것이 더 적절할 때 직접 정의를 풀어서 설명하는 대신 인용하도록 수정합니다. 또한, 공무원 징계에 관한 내용은 관계 법령에 따라 할 수 있으므로 조례에 재기재할 실익이 낮다고 판단하여 삭제를 권고할 수 있습니다.\n"
        "- 위임 범위 준수: 위임조례의 경우, 상위 법령이 위임한 범위 내에서 조례가 규정되었는지 확인하고, 그 범위를 벗어나는 내용은 수정하거나 삭제를 제안합니다.\n"
        "- 경과조치 등 부칙 추가: 전부개정의 경우, 기존 조례의 법적 효과를 명확히 하기 위해 기금, 관리 위탁, 심의위원회 등에 관한 경과조치 부칙 규정을 추가할 것을 권고합니다.\n\n"
        "2. 용어의 명확성 및 통일성:\n"
        "- 정의 규정의 명확화: 조례에서 사용되는 핵심 용어의 정의를 명확히 하도록 제안합니다. 예를 들어, '갑질 행위'의 정의를 명확히 하거나, '못난이 농산물 상표'의 종류와 형상에 대한 구체적인 규정을 제시할 수 있습니다. '다자녀가정'의 정의를 출산 또는 입양 등으로 두 명 이상의 자녀를 양육하는 가정으로 명확히 하는 것과 같이.\n"
        "- 불필요한 용어 정의 삭제: 사회 통념상 충분히 의미를 알 수 있는 용어(예: '갑질 행위자')는 정의 규정에서 삭제할 것을 권고합니다.\n"
        "- 문구 수정 및 표준화: 애매하거나 불필요한 수식어를 제거하고 간결하고 명확한 문장으로 수정합니다. 예를 들어, '책무' 조문의 문구를 수정하여 조례의 목적과 일치시키는 것.\n\n"
        "3. 정책 의도 반영 및 실효성 제고 :\n"
        "- 지원 대상 및 범위 명확화: 지원 대상이나 지원 범위가 불명확한 경우 구체적인 대상을 명시하도록 제안합니다. 예를 들어, '안전취약계층'의 범위를 구체적인 법적 기준에 따라 명시하거나, '청년'의 정의를 명확히 하는 것.\n"
        "- 필수 누락 사항 제안: 시행일, 위원회 간사 규정, 평가 주기 명시 등 조례의 실효적 운영을 위해 필요한데 누락된 사항을 추가할 것을 제안합니다.\n"
        "- 실익 검토 및 불필요한 규정 삭제: 특정 규정이 조례에 포함될 실익이 적거나 행정 규칙 등으로 정할 수 있는 내용이라면 삭제를 권고합니다. 예를 들어, 공공시설 매점 계약에서 우선계약자의 순위를 조례로 정하는 것이 상위법령에 위배될 소지가 있어 재검토를 권고하는 것.\n"
    )
    return prompt
